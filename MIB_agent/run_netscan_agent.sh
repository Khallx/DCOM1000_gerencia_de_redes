...
TMPDIR="$(mktemp --directory --tmpdir netscan_agent.XXXXXXXXXX)"
...
echo ""
echo "    cd `pwd`"
echo "    snmpwalk -v 2c -c public -M+. localhost:5555 NETSCAN-MIB::netscanMIB"
echo "    snmptable -v 2c -c public -M+. -Ci localhost:5555 NETSCAN-MIB::deviceTable"
echo "    snmpget -v 2c -c public -M+. localhost:5555 NETSCAN-MIB::nsScanPeriod.0"
echo "    snmpset -v 2c -c simple -M+. localhost:5555 NETSCAN-MIB::nsScanPeriod.0 u 10"
echo ""
...
echo "* Starting the netscan agent..."
python netscan_agent.py -m $TMPDIR/snmpd-agentx.sock -p $TMPDIR/
